...
# number of parameters in the neural ODE chosen (to be modified) to enforce the invariance
...
acs = np.load('./data/acs.npy')
lidar = torch.tensor(obs).float()  # batch, seq, data
control = torch.tensor(acs).float()
...
def get_batch(bitr):  # get training data from trajectory bitr
    # obs.shape = (100, 51, 1, 19)
    control = control_bat[bitr, :, :, :]
    lidar = lidar_bat[bitr, :, :, :]  # [51, 1, 19]
    bb = torch.from_numpy(np.random.choice(np.arange(
        args.data_size - args.batch_time, dtype=np.int64), args.batch_size-1, replace=False))
    # bb.shape = [19], aa.shape = [1]
    aa = torch.tensor([0])   # augment 0
    s = torch.cat([bb, aa], dim=0)  # s.shape = [20]
    batch_u0 = control[s]  # (M, D) # [20, 1, 5]
    batch_lidar0 = lidar[s]  # [20, 1, 19]
    batch_t = t[:args.batch_time]  # (T)
    batch_u = torch.stack([control[s + i]
                          for i in range(args.batch_time)], dim=0)  # (T, M, D)
    batch_lidar = torch.stack([lidar[s + i]
...
def visualize(true_y, pred_y, itr):
    ...
    if args.viz:
        ...
        ax_traj.plot(t.cpu().numpy(), true_y.cpu().numpy()[
                     :, 0, 0], t.cpu().numpy(), true_y.cpu().numpy()[:, 0, 1], 'g-')
        ax_traj.plot(t.cpu().numpy(), pred_y.cpu().numpy()[
                     :, 0, 0], '--', t.cpu().numpy(), pred_y.cpu().numpy()[:, 0, 1], 'b--')
        ...
        ax_phase.plot(true_y.cpu().numpy()[
                      :, 0, 0], true_y.cpu().numpy()[:, 0, 1], 'g-')
        ax_phase.plot(pred_y.cpu().numpy()[
                      :, 0, 0], pred_y.cpu().numpy()[:, 0, 1], 'b--')
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        if self.training:
            ...
            g = self.net(y)  # [20, 1, 100]
            u1, u2, u3, u4, u5 = torch.chunk(g[:, :, 5:], 5, dim=-1)

            if args.use_fc == True:
                sensor = torch.cat(x, dim=2)  # add theta and speed
            else:
                sensor = self.lidar
                # sensor = torch.cat(self.lidar, dim = 2)
            out = torch.cat([
                (u1*sensor).sum(axis=2).unsqueeze(2),
                (u2*sensor).sum(axis=2).unsqueeze(2),
                (u3*sensor).sum(axis=2).unsqueeze(2),
                (u4*sensor).sum(axis=2).unsqueeze(2),
                (u5*sensor).sum(axis=2).unsqueeze(2)
            ], dim=2) + g[:, :, 0:5]  # \dot x = g1(x) + g2(x)u
        else:
            if args.use_fc == True:
                x = self._fc(self.lidar)
            g = self.net(y)
            u1, u2, u3, u4, u5 = torch.chunk(g[:, 5:], 5, dim=-1)  # [1, 19]

            if args.use_fc == True:
                sensor = torch.cat(x, dim=1)  # add theta and speed
            else:
                sensor = self.lidar  # [1, 19]
                # sensor = torch.cat(self.lidar, dim = 1)
            out = torch.cat([
                (u1*sensor).sum(axis=1).unsqueeze(1),
                (u2*sensor).sum(axis=1).unsqueeze(1),
                (u3*sensor).sum(axis=1).unsqueeze(1),
                (u4*sensor).sum(axis=1).unsqueeze(1),
                (u5*sensor).sum(axis=1).unsqueeze(1)
            ], dim=1) + g[:, 0:5]  # \dot x = g1(x) + g2(x)u

        return out

    # used in testing, u-output of neural ODE, I-lidar info., ego-ego state, other-other vehicle state, the remainings are CBF constraints related.
    def dCBF(self, u, I, f, g1_complete, g2_complete, ego, other):
        ...
        f1_state, f2_state = (g1_complete[-2:]*state[0, 2:4]).sum(
            axis=0), (g2_complete[-2:]*state[0, 2:4]).sum(axis=0)
        dim = g1.shape[0]
...
if __name__ == '__main__':
    ...
    if args.use_cnn or args.use_fc:
        # [2, 32, 64, 26]   # +ego speed, ego heading
        fc_param = [2, 64, 128, 30]
    else:
        fc_param = [5, 64, 256, 512, 100]
        # fc_param = [2, 50, 400, 206]
        # fc_param = [2, 64, 256, 512, 206] # 202 * 2 + 2
    ...
    for itr in range(1, args.niters + 1):
        for bitr in range(100):  # 100 trajectories
            func.use_dcbf = False
            func.train()
            optimizer.zero_grad()
            ...
            for i in range(args.batch_time-1):
                # print('iteration: ', itr, '| traj: ', bitr, '| step:', i, 'test loss: ', test_loss.item(), 'train loss:', loss.item())
                ...
                # , method = 'explicit_adams' euler, midpoint, rk4, explicit_adams, fixed_adams
                ...
                pred_u = torch.cat(
                    [pred_u, pred[-1, :, :, :].unsqueeze(0)], dim=0)
                u0 = pred[-1, :, :, :]

            print('iteration: ', itr, '| traj: ', bitr, '| test loss: ',
                  test_loss.item(), '| train loss:', loss.item())

            if args.use_diff_weight:
                loss = 0.01*torch.mean(torch.abs(pred_u[:, :, :, 0] - batch_u[:, :, :, 0])) + 0.99*torch.mean(
                    torch.abs(pred_u[:, :, :, 1] - batch_u[:, :, :, 1]))
            else:
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                for i in range(args.data_size-1):
                    ...
                    u0 = pred[-1, :, :]
                if args.use_diff_weight:
                    test_loss = 0.01*torch.mean(torch.abs(pred_u[:, :, 0] - control_test[:, :, 0])) + 0.99*torch.mean(
                        torch.abs(pred_u[:, :, 1] - control_test[:, :, 1]))
                else:
            ...
            if args.consider_noise:
                torch.save(
                    func.state_dict(), "./models_noise/model_node_itr" + format(ii, '02d') + ".pth")
            else:
                torch.save(func.state_dict(), "./models_cbf" + exp_id +
                           "/model_node_itr" + format(ii, '02d') + ".pth")
